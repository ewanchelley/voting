...
export class PopularComponent implements OnInit {
  ...
  weakly() {
      ...
      for (let r = 0; r < m; r++) {
        for (let s = r + 1; s < m; s++) {
          ...
          if(r_before_s){
            ...
            model.push(`R${i}_${s}_${r} = 0`);
          } else {
            model.push(`R${i}_${s}_${r} = 1`);
    ...
    // Add constraint 1 + x{r,t} >= x{r,s} + x{s,t}
    for (let r = 0; r < m; r++) {
      for (let s = 0; s < m; s++) {
        for (let t = 0; t < m; t++) {
          if (r != s && s != t && r != t) {
            model.push(`1 + x${r}_${t} >= x${r}_${s} + x${s}_${t}`);
          }
        }
      }
    }
      ...
      for (let r = 0; r < m; r++) {
        for (let s = 0; s < m; s++) {
          if(r != s){
            ...
            model.push(`k${i}_${r}_${s} >= x${r}_${s} - R${i}_${r}_${s}`);
            model.push(`k${i}_${r}_${s} >= R${i}_${r}_${s} - x${r}_${s}`);
            model.push(`k${i}_${r}_${s} <= x${r}_${s} + R${i}_${r}_${s}`);
            model.push(`k${i}_${r}_${s} <= 2 - x${r}_${s} - R${i}_${r}_${s}`);
          }
        }
      }
    }
    ...
    for (let i = 1; i <= n; i++) {
      ...
      model.push(`p${i} >= (K${i} - K${i}PRIME) / (${m}(${m}-1))`);
      model.push(`K${i} - K${i}PRIME >= p${i} - ${m}(${m}-1)(1 - p${i})`);
    }
